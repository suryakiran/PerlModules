...
if !hasmapto("<Plug>Calendar")
  nmap <unique> <Leader>vc <Plug>Calendar
endif
if !hasmapto("<Plug>CalendarH")
  nmap <unique> <Leader>hc <Plug>CalendarH
endif
...
function! s:CalendarDoAction()
  ...
  " for navi
  if exists('g:calendar_navi')
    if expand("<cword>") == 'Prev'
	  exec substitute(maparg('<s-left>', 'n'), '<CR>', '', '')
      return
    elseif expand("<cword>") == 'Next'
...
function! Calendar(...)
  ...
  if exists('g:calendar_navi')
    if dir
      let navcol = ((vcolumn/2)*3-4)
    else
      let navcol = ((vcolumn/2)-4)
    endif

    if g:calendar_navi == 'top'
      execute "normal gg".navcol."i "
      silent exec "normal! i<Prev Next>\<cr>\<cr>"
    ...
    if g:calendar_navi == 'bottom'
      ...
      silent exec "normal! i<Prev Next>"
    endif
    if g:calendar_navi == 'both'
      execute "normal gg".navcol."i "
      silent exec "normal! i<Prev Next>\<cr>\<cr>"
