...
" Last Change:	: 2002/11/01 15:33:29 .
...
"	See MonthString() for special extension of format.
"	Default: '%d-%3m-%Y'
"
"   'autodate_lines'
...
"	|strftime()|を参照。フォーマットへの独自拡張についてはMonthString()
"	を参照。省略値: '%d-%3m-%Y'
"
"   'autodate_lines'
...
if has("autocmd")
  augroup Autodate
    au!
    autocmd BufUnload,FileWritePre,VimLeavePre,BufWritePre * call <SID>Autodate()
  augroup END
endif " has("autocmd")
...
function! Strftime2(...)
  ...
  if a:0 > 0
    ...
    if a:1 =~# '%\d\+m'
      let format = substitute(a:1, '%\d\+m', MonthString(-1, matchstr(a:1, '\(%\)\@<=\d\+\(m\)\@='), time), 'g')
    else
      let format = a:1
    endif
...
"	例:
"	    :echo MonthString(8) . " 2001"
"	    August 2001
"	    :echo MonthString(8,3) . " 2001"
"	    Aug 2001
"
